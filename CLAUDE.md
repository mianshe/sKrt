...
- **PWA 路线**：PWA 不再区分“本地处理 / 云端处理”，统一按**云端解析、云端向量化、云端建索引、云端聊天**来设计，上传后应始终能看到处理进度。
- **未来客户端路线**：后续新增的 **`exe` / `apk`** 版本，核心目标是承接“**过程文件尽量存用户本地**”的能力，包括原件、本地过程文件、本地缓存等；这也是做桌面端/移动端壳而不是只维持 PWA 的主要原因。
- **实现边界**：如需真正做到“知识和过程文件主要保存在用户本地，同时仍可继续使用处理后的知识”，优先在 **桌面端 / 原生客户端** 实现，不要把该目标继续压在纯网页 PWA 上。
- **当前客户端选型**：桌面端优先使用 **Tauri**；安卓端优先使用 **Capacitor**。不要再把 **TWA** 作为“本地能力客户端”的默认路线。
